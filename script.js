...
const RESERVATION_API_URL =
  'https://dmambuvhtfyacrpvjnmy.supabase.co/functions/v1/reservations';
...
function initReservationForm() {
  ...
  const validators = {
    ...
    date: (value) =>
      value.trim() !== '' || 'Please choose a date.',

    time: (value) =>
      value.trim() !== '' || 'Please choose a time.',
  };
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      if (!response.ok) {
        if (data.fields) {
          showFieldErrorsFromServer(data.fields);
        }
